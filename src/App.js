        ...
        return (
            <div className="slidecontainer">
                ...
                <input type="range" min={this.props.min} max={this.props.max}
                           ...
                           value={this.props.value} step={this.props.step} className="slider" ref={this.props.refName} />
                <span>{value}</span>
                {slideInfo}
            </div>
        );
...
class HistoricTable extends Component {
    ...
    getRows() {
        ...
        return this.props.data.map((item, index) => {
            ...
            let reactivationInfo = item.reactivated > 0 ? <small>-${moneyFormatter(reactivationCost)}</small> : null;
            return <tr key={index}>
                <td>{index + 1}</td>
                <td>
                    {item.monthlyNew}
                ...
                <td>
                    {item.reactivated}{reactivationInfo}
                </td>
                <td>
                    {item.totalUsers}
                ...
                <td>
                    <span className={"text-muted"}>&Sigma;</span>{moneyFormatter(totalRevenue)}
...
class App extends Component {
    ...
    render() {
        ...
        let percentageFormatter = (val) => { return parseInt(val * 100) + '%';};
    return (
